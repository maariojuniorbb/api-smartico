...
export async function atualizarJogadorLote(jogadores: JogadorPreferencia[]): Promise<void> {
  ...
  logger.info(`iniciando a atualização de ${payload.length} jogadores no Smartico`);
  try {
    ...
    logger.info('Lote de jogadores atualizado com sucesso');
  } catch (error: any) {
    logger.error('Erro ao atualizar lote de jogadores: %o', error.response?.data || error.message);
    throw new Error('Erro ao atualizar lote de jogadores');
  }
}
